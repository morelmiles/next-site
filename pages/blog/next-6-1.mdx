...
A change we've made that is more visible to users is that `.next/dist` has been renamed to `.next/server`. The `.next` directory that holds build output. For example when you run `next build` the result will be stored in `.next`.<Image src={'https://assets.zeit.co/image/upload/front' + '/blog/next6-1/server-dist.png'} width={1574/3} height={754/3} />
<Caption>
